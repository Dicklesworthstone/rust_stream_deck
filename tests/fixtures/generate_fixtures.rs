//! Fixture generation script for Stream Deck CLI tests.
//!
//! Run with: cargo run --example generate_fixtures
//!
//! This script generates all test images and data files needed for the test suite.
...
fn generate_valid_images(base: &Path) {
    ...
    create_solid_image(&valid_dir.join("large-256x256.png"), 256, 256, [200, 100, 150]);
    create_solid_image(&valid_dir.join("large-1024x1024.png"), 1024, 1024, [50, 100, 150]);
    create_solid_image(&valid_dir.join("small-50x50.png"), 50, 50, [100, 200, 100]);
    create_solid_image(&valid_dir.join("nonsquare-100x80.png"), 100, 80, [200, 200, 100]);
    ...
    println!("Generated valid test images");
}
...
fn generate_invalid_images(base: &Path) {
    ...
    fs::write(invalid_dir.join("not-image.txt.png"), b"This is not an image file")
    ...
    fs::write(invalid_dir.join("truncated.png"), &truncated).expect("Failed to write truncated file");
    ...
    println!("Generated invalid test images");
}
...
fn generate_batch_images(base: &Path) {
    ...
    println!("Generated batch test images");
}
...
fn generate_color_images(base: &Path) {
    ...
    println!("Generated color test images");
}
...
fn create_numbered_image(path: &Path, width: u32, height: u32, color: [u8; 3], number: u8) {
    ...
    let dot_size = 4;
    for i in 0..=number {
        let angle = (i as f32 * 360.0 / 32.0).to_radians();
        let radius = 20.0;
        let dx = (angle.cos() * radius) as i32;
        let dy = (angle.sin() * radius) as i32;

        let px = (center_x as i32 + dx).clamp(0, width as i32 - 1) as u32;
        let py = (center_y as i32 + dy).clamp(0, height as i32 - 1) as u32;
...
fn key_color(index: u8) -> [u8; 3] {
    ...
    let hue = (index as f32 * 360.0 / 32.0) % 360.0;
